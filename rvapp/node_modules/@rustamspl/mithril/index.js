...
(function(global, factory) { // eslint-disable-line
        ...
        m.route = function(root, arg1, arg2, vdom) { // eslint-disable-line
            ...
            if (arguments.length === 3 && isString(arg1)) {
                redirect = function(source) {
                    var path = currentRoute = source
                    //         console.log('xxxx',source,'[', arg1,'][',arg2,']', path);
                    if (!routeByValue(root, arg2, path)) {
                        if (isDefaultRoute) {
                            throw new Error("Ensure the default route matches " + "one of the routes defined in m.route. [" + source + "]")
                        }
                        isDefaultRoute = true
                        m.route(arg1, true)
                        isDefaultRoute = false
                        //          console.log('zzzz',path);
                    }
                }
                var listener = "onpopstate" //m.route.mode === "hash" ? "onhashchange" :
            ...
            if (root.addEventListener || root.attachEvent) {
                // console.log('root.addEventListener || root.attachEvent');
                //var base = m.route.mode !== "pathname" ? $location.pathname : ""
                root.href = vdom.attrs.href //base + modes[m.route.mode] + 
                if (root.addEventListener) {
                    root.removeEventListener("touchend", routeUnobtrusive)
                    root.removeEventListener("click", routeUnobtrusive)
                    root.addEventListener("touchend", routeUnobtrusive)
            ...
            if (isString(root)) {
                ...
                var queryIndex = 0
                var params
                params = parseQueryString(currentRoute)
                ...
                if (querystring) {
                    currentRoute = $location.pathname + '?' + querystring //currentPath + (currentPath.indexOf("?") === -1 ? "?" : "&") +
                } else {
                ...
                if (global.history.pushState) {
                    computePostRedrawHook = function() {
                        try {
                            global.history.replaceState(null, $document.title, currentRoute)
                        } catch (err) {
    ...
    function routeByValue(root, router, path) {
        routeParams = {}
        routeParams = parseQueryString(path)
    ...
    function routeUnobtrusive(e) {
        ...
        var args
        // console.log('currentTarget.search');
        ...
        pendingRequests = 0
        m.route(currentTarget.search, args)
        // try{
        //     e.stopPropagation(); 
        //     e.preventDefault(); 
        // }catch(e){
    ...
    function handleJsonp(options) {
        var callbackKey = options.callbackName || "_callback_" + new Date().getTime() + "_" + (Math.round(Math.random() * 1e16)).toString(36)
